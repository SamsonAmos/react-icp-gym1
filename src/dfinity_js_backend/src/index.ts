...
export default Canister({
  ...
  updateGymById: update(
    ...
    (id, payload) => {
      ...
      if ("None" in gymOpt) {
        return Err({
          NotFound: `cannot update gym: gym with id=${id} not found`,
        });
      }
  ...
  gymMembershipRegistration: update(
    [MembershipPayload],
    Result(Gym, Message),
    (payload) => {
      ...
      gymOpt.Some.members.push({
        userId: ic.caller().toText(),
        userName,
        gymId,
        fullName,
        emailAddress,
  ...
  getAllEnrollesByGymId: query(
    [text],
    Result(Vec(MembershipPayload), Message),
    (id) => {
      if (!isValidUuid(id)) {
        return Err({
  ...
  addGymService: update(
    ...
    (payload) => {
      ...
      if (gymOpt.Some.owner.toText() !== ic.caller().toText()) {
        return Err({
          NotAuthorized: `you are not the owner of this gym with id=${gymId} `,
        });
      }

      gymOpt.Some.gymServices.push({
        gymId,
        serviceName,
        serviceDescription,
        operatingDaysStart,
  ...
  getAllServicesById: query(
    [text],
    Result(Vec(GymServicePayload), Message),
    (id) => {
      if (!isValidUuid(id)) {
        return Err({
          InvalidPayload: `id=${id} is not in the valid format.`,
